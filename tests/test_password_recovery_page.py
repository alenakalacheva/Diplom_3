...
class TestRecoveryPage:
    ...
    def test_follow_to_the_password_recovery_page(self, driver):
        ...
        assert recovery_page.check_recovery_form() and recovery_page.get_current_url() == (Urls.MAIN_PAGE_URL + Urls.RECOVERY_PAGE)
    ...
    def test_input_password_and_click_recovery_btn(self, driver):
        ...
        main_page.move_to_personal_account_btn_and_click()
        login_page.click_recovery_btn()
        recovery_page.send_email_to_email_field(helpers.create_correct_user_data()["email"])
        recovery_page.click_recovery_btn()
        assert recovery_page.check_save_btn() and recovery_page.get_current_url() == (Urls.MAIN_PAGE_URL + Urls.RESET_PAGE)
    ...
    def test_checking_the_backlight_of_the_password_field(self, driver):
        ...
        login_page = LoginPage(driver)
        recovery_page = PasswordRecoveryPage(driver)
        person_data = helpers.create_correct_user_data()
        main_page.move_to_personal_account_btn_and_click()
        login_page.click_recovery_btn()
